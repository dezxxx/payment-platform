...
Взять `traceId` из строки «Registered» в Loki и вставить его в Tempo — откроется
именно этот трейс. В Grafana поле `TraceID` в логе кликабельно, переход
настроен в провижнинге источников.
