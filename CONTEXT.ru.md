...
### Всё ещё вводит в заблуждение: 401 про неверный пароль

`ApiAuthenticationEntryPoint` переводит любой отказ фильтра в
`INVALID_CREDENTIALS`, поэтому опечатка в URL отвечает «Email or password is
incorrect» на запрос, в котором пароля не было вовсе. Статус верный —
**401 (Unauthorized)** это то, что resource server должен неаутентифицированному
клиенту, — а сообщение нет. Просится отдельный `ErrorCode` на «нет валидного
токена», а `INVALID_CREDENTIALS` оставить логину, где пароль действительно
сравнивали.
...
- [ ] Публикация `person-client` в Nexus — критерий приёмки 6; пока отработан
      только `publishToMavenLocal`. Самого Nexus нет и в `docker-compose.yml`, а
      его URL `localhost:8083` изнутри контейнера означает сам контейнер —
      понадобится имя compose-сервиса
- [ ] Отдельный `ErrorCode` на «нет валидного токена», чтобы опечатка в URL
      перестала отвечать «Email or password is incorrect» — см. выше
